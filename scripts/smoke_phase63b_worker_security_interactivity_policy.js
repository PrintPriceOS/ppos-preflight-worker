...
const ENGINE_REPORT_PATH = path.resolve(__dirname, '../../ppos-preflight-engine/reports/phase63a_engine_security_interactivity_fixes.json');

// Map an Engine 63A regression result entry into a synthetic Engine autofixPdf-shaped payload.
...
async function runSmokeTest() {
    ...
    if (await fs.pathExists(ENGINE_REPORT_PATH)) {
        ...
        console.log(`[TEST] Loaded ${engineScenarios.length} scenarios from Engine report (63A).`);
    } else {
        console.log('[TEST] Engine 63A report not found — using SYNTHETIC_POLICY_FALLBACK scenarios.');
        engineScenarios = [
    ...
    const testDir = path.join(os.tmpdir(), `phase63b-worker-test-${Date.now()}`);
    ...
    const reportJsonPath = path.join(REPORTS_DIR, 'phase63b_worker_security_interactivity_policy.json');
    const reportMdPath = path.join(REPORTS_DIR, 'phase63b_worker_security_interactivity_policy.md');
    ...
    let md = `# Phase 63B — Worker Security/Interactivity Fix Audit Policy\n\n`;
