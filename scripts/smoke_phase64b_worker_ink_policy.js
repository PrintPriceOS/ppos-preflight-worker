...
// Map an Engine 64A regression result entry into a synthetic Engine autofixPdf-shaped payload.
...
async function runSmokeTest() {
    ...
    if (await fs.pathExists(ENGINE_REPORT_PATH)) {
        ...
        console.log(`[TEST] Loaded ${engineScenarios.length} scenarios from Engine report (64A).`);
    } else {
        console.log('[TEST] Engine 64A report not found — using SYNTHETIC_POLICY_FALLBACK scenarios.');
        engineScenarios = [
            { name: 'SYNTHETIC: REDUCE_TOTAL_INK_COVERAGE skipped unsupported', mockEngineResult: { applied_fixes: [], skipped_fixes: [{ fix_id: 'REDUCE_TOTAL_INK_COVERAGE', status: 'SKIPPED_UNSUPPORTED', reason: 'SKIPPED_UNSUPPORTED', evidence: { implemented: false } }], failed_fixes: [] } },
            { name: 'SYNTHETIC: MAP_RICH_BLACK_TEXT_TO_K_ONLY skipped unsupported', mockEngineResult: { applied_fixes: [], skipped_fixes: [{ fix_id: 'MAP_RICH_BLACK_TEXT_TO_K_ONLY', status: 'SKIPPED_UNSUPPORTED', reason: 'SKIPPED_UNSUPPORTED', evidence: { implemented: false } }], failed_fixes: [] } },
            { name: 'SYNTHETIC: MAP_REGISTRATION_COLOR_TO_BLACK skipped unsupported', mockEngineResult: { applied_fixes: [], skipped_fixes: [{ fix_id: 'MAP_REGISTRATION_COLOR_TO_BLACK', status: 'SKIPPED_UNSUPPORTED', reason: 'SKIPPED_UNSUPPORTED', evidence: { implemented: false } }], failed_fixes: [] } },
            { name: 'SYNTHETIC: NORMALIZE_BLACK_TEXT skipped unsupported', mockEngineResult: { applied_fixes: [], skipped_fixes: [{ fix_id: 'NORMALIZE_BLACK_TEXT', status: 'SKIPPED_UNSUPPORTED', reason: 'SKIPPED_UNSUPPORTED', evidence: { implemented: false } }], failed_fixes: [] } },
            { name: 'SYNTHETIC: DETECT_SMALL_TEXT_RICH_BLACK detected', mockEngineResult: { applied_fixes: [{ fix_id: 'DETECT_SMALL_TEXT_RICH_BLACK', status: 'APPLIED', evidence: { small_text_rich_black_regions: 2 } }], skipped_fixes: [], failed_fixes: [] } },
            { name: 'SYNTHETIC: mixed ink fixes (TAC + rich black)', mockEngineResult: { applied_fixes: [{ fix_id: 'DETECT_SMALL_TEXT_RICH_BLACK', status: 'APPLIED', evidence: { small_text_rich_black_regions: 1 } }], skipped_fixes: [{ fix_id: 'REDUCE_TOTAL_INK_COVERAGE', status: 'SKIPPED_UNSUPPORTED', reason: 'SKIPPED_UNSUPPORTED', evidence: { implemented: false } }], failed_fixes: [] } },
    ...
    engineScenarios.push(
        {
            name: 'REGRESSION: standards overclaim from ink fix must be rejected',
            mockEngineResult: {
                applied_fixes: [{ fix_id: 'MAP_RICH_BLACK_TEXT_TO_K_ONLY', status: 'APPLIED', evidence: { rich_black_text_regions: 1 } }],
                skipped_fixes: [],
                failed_fixes: [],
        ...
        },
        {
            ...
            mockEngineResult: {
                applied_fixes: [{ fix_id: 'NORMALIZE_BLACK_TEXT', status: 'APPLIED', evidence: { black_text_regions_normalized: 1 } }],
                skipped_fixes: [],
                failed_fixes: [],
        ...
        },
        {
            ...
            mockEngineResult: {
                applied_fixes: [{ fix_id: 'MAP_REGISTRATION_COLOR_TO_BLACK', status: 'APPLIED', evidence: { registration_color_regions_mapped: 2 } }],
                skipped_fixes: [{ fix_id: 'REDUCE_TOTAL_INK_COVERAGE', status: 'SKIPPED_UNSUPPORTED', reason: 'SKIPPED_UNSUPPORTED', evidence: { implemented: false, limitations: ['No rendering pipeline available for safe TAC reduction.'] } }],
                failed_fixes: [{ fix_id: 'NORMALIZE_BLACK_TEXT', status: 'FAILED', evidence: { black_text_regions_normalized: 0, warnings: ['Tool error while rewriting text color.'] } }]
            }
        }
    );
    ...
    const testDir = path.join(os.tmpdir(), `phase64b-worker-test-${Date.now()}`);
    ...
    for (const scenario of engineScenarios) {
        ...
        const job = {
            data: {
                jobId: 'job-64b',
                tenantId: 'tenant-64b',
                input: { fileUrl: mockInput, findings: [{ id: 'IND_INK_001' }] },
                fixes: ['REDUCE_TOTAL_INK_COVERAGE']
            },
        ...
        try {
            ...
            const gov = audit.ink_governance;
            const trust = audit.artifact_trust;

            if (!gov) { pass = false; notes.push('ink_governance missing in fix_audit.json'); }
            if (!delta.ink_governance) { pass = false; notes.push('ink_governance missing in delta_report.json'); }
            ...
            const hasInkActivity = allFixes.length > 0;

            if (gov && hasInkActivity) {
                if (!gov.review_required) { pass = false; notes.push('Review not required when ink fix activity present'); }
                ...
                if (hasApplied && !gov.visual_change_expected) {
                    pass = false;
                    notes.push('visual_change_expected not set true when ink fix applied');
                }
            }
            ...
            if (scenario.name.startsWith('REGRESSION: standards overclaim')) {
                ...
                if (sc.pdfx_compliance_claimed || sc.pdfa_compliance_claimed || sc.compliance_claim_allowed || trust.standard_certified || trust.production_certified) {
                    pass = false;
                    notes.push('Worker propagated a standards/production overclaim from an ink fix');
                }
            }
    ...
    const reportJsonPath = path.join(REPORTS_DIR, 'phase64b_worker_ink_policy.json');
    const reportMdPath = path.join(REPORTS_DIR, 'phase64b_worker_ink_policy.md');
    ...
    let md = `# Phase 64B — Worker Ink Governance Policy\n\n`;
