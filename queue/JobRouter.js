...
class JobRouter {
    ...
    static async route(job, logger = console) {
        ...
        if (!tenantId || !jobId) {
            throw new Error(`CRITICAL: Job ${job.id} has incomplete contract. Isolation boundary breached.`);
        }
        ...
        const entityId = input?.assetId || jobId;
        ...
        try {
            ...
            switch (job.name) {
                ...
                case 'AUTOFIX':
                    ...
                    break;
                default:
                    // Support the generic "preflight_pipeline" trigger if used
                    result = await AnalyzeProcessor.process(job.data, logger);
            }
